...
/*   Updated: 2025/12/06 10:15:38 by natakaha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char **arglist, t_flag *flaglist)
{
	...
	if (flaglist[pindex].format != 'c'
		...
		charcount = ft_putstr_fd_c(arglist[pindex], 1);
	else if (flaglist[pindex].format == '%')
		charcount = ft_putchar_fd_c(arglist[pindex][0], 1);
	else
	...
	char **arglist, t_flag *flaglist)
{
	...
	while (*format)
	{
		...
		if (*format == '%')
		{
			...
			charcount = ft_putchar_fd_c(*format++, 1);
		if (charcount < 0)
			break ;
		sum += charcount;
	}
